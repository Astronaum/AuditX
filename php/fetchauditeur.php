...
if(isset($_POST["query"]))
{
 ...
 $query = "
  SELECT * FROM audits 
  WHERE nom_audit LIKE '%".$search."%'
  OR prenom_audit LIKE '%".$search."%'
  OR bureau_audit LIKE '%".$search."%'
 ";
}
else
{
...
if(mysqli_num_rows($result) > 0)
{
 $output .= '
    ...
    <tr><th scope="col" class="text-muted">Nom</th><th scope="col" class="text-muted">Prénom</th><th scope="col" class="text-muted">Bureau</th><th scope="col"></th><th scope="col"></th>
    <tbody>
    </tr>
 ';
 while($row = mysqli_fetch_array($result))
 {
  $output .= '
   <tr>
    <td>'.$row["nom_audit"].'</td>
    <td>'.$row["prenom_audit"].'</td>
    <td>'.$row["bureau_audit"].'</td>
    <td><a class="text-decoration-none text-dark fw-bold " href=editeraudit.php?id_audit='.$row["id_audit"].' >Editer</a></td>
    <td><a class="text-decoration-none text-danger fw-bold" href=supprimeraudit.php?id_audit='.$row["id_audit"].' >Supprimer</a></td>
    </tr>
  ';
 }
 echo $output;
