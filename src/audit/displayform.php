...
if(!isset($_SESSION["id_user"]))
    ...
    $id_collab=$_GET['id_coll'];
    $sql1="SELECT * FROM collaborateurs WHERE id_collaborateur='$id_collab'";
    $result1=mysqli_query($link,$sql1);
    $row1=mysqli_fetch_assoc($result1);
    $role=$row1['id_role'];
    $sql5="SELECT * from roles where id_role='$role'";
    $result5 = mysqli_query($link, $sql5);
    $row5 = mysqli_fetch_array($result5);
    $role_lib=$row5['libelle_role'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <div id="content">
                ...
                <div class="d-flex justify-content-center mt-5">
                    <div class="card ms-2 shadow rounded-0">
                            <div class="card-body">
                            <p class="card-text text-danger fw-bold">Questionnaire à <?php echo $row1['nom_collaborateur'].' '.$row1['prenom_collaborateur'].' ('.$role_lib.')'; ?></p>
                            </div>
                    </div>
                </div>
                <form class="row g-3 m-5 p-5 pt-0" method="post" enctype="multipart/form-data">
                    <?php
                        $sql3="SELECT * from formulaires WHERE id_audit='$id_audit_cour'AND role='$role'";
                        $result3=mysqli_query($link,$sql3);
                        $row3=mysqli_fetch_assoc($result3);
                        if($row3==NULL){

                                echo '<div class="d-flex justify-content-center mt-5">
                                        <p class="card-text text-danger fw-bold">Pas de questionnaire pour cette fonction <a href="creerformulaire.php" class="text-success">Veuillez créer un.</a></p>
                                </div>
                            </div>';
                        }else{
                            echo '<div class="col-md-5">
                                <label class="form-label text-center">Question</label>    
                            </div>
                            <div class="col-md-2">
                                <label class="form-label text-center">Type de réponse</label>                
                            </div>
                            <div class="col-md-5">
                                <label class="form-label text-center">Réponse</label>          
                            </div> ';
                        $recup_id_from=$row3['id_formulaire'];

                        $sql4="SELECT * from questions where id_formulaire='$recup_id_from'";
                        $result4=mysqli_query($link,$sql4);
                        while($row4 = mysqli_fetch_array($result4)){
                            
                            echo '  <div class="col-md-5">
                                    <input type="text" class="form-control weirdfocus" value="'.$row4['question'].'" disabled>
                                    </div>
                                    <div class="col-md-2">
                                ';
                                $r=$row4['type_reponse'];
                            if($r==0){
                                    echo '  <input type="text" class="form-control weirdfocus" value="Texte" disabled></div>
                                            <div class="col-md-5">
                                            <input type="text" class="form-control weirdfocus" name="'.$row4['id_question'].'">';
                            }elseif ($r==1){
                                        echo '  <input type="text" class="form-control weirdfocus" value="Oui/Non" disabled></div>
                                                <div class="col-md-5">
                                                <select type="text" class="form-control weirdfocus" name="'.$row4['id_question'].'">
                                                    <option value="Oui">Oui</option>
                                                    <option value="Non">Non</option>
                                                </select>';
                            }else{
                                        echo '<input type="text" class="form-control weirdfocus" value="Pourcentage" disabled></div>
                                              <div class="col-md-5">
                                              <input type="number" class="form-control weirdfocus" name="'.$row4['id_question'].'" min="0" max="100">';
                            }
                            echo '</div>';
                        };
...
if(isset($_POST['enregistrer'])){
    ...
    while($row4 = mysqli_fetch_array($result4)){
        ...
        $rep=$_POST[$row4['id_question']];
        $id_qs=$row4['id_question'];
        $sql5="INSERT INTO reponses VALUES (NULL,'$recup_id_from','$id_qs','$id_collab','$rep')";
        $result5=mysqli_query($link,$sql5);

    }

    echo "<script>window.location.replace(\"auditer.php\");</script>";
