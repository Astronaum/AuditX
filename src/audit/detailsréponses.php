<?php
    ...
    $id_a_c=$_SESSION['id_a_c'];

    $sql0="SELECT * FROM collaborateurs WHERE id_collaborateur='$id_collab'";
...
<head>
    ...
    <title>AuditX - Détails des réponses</title>
            ...
            <div id="content">
                ...
                <div class="d-flex justify-content-center mt-5">
                    <div class="card ms-2 shadow rounded-0">
                            <div class="card-body">
                            <p class="card-text text-danger fw-bold">Fiche de réponse du collaborateur : <?php echo $row0['nom_collaborateur'].' '.$row0['prenom_collaborateur']?></p>
                            </div>
                    </div>
                </div>
                    ...
                    $sql1="SELECT * FROM formulaires WHERE role='$id_role' AND id_audit='$id_a_c'";
                    $result1=mysqli_query($link,$sql1);
                    if ($result1==NULL){

                        echo '<div class="d-flex justify-content-center mt-5">
                            <p class="card-text text-danger fw-bold">Collaborateur non audité. <a href="auditer.php" class="text-success">Veuillez l\'auditer.</a></p>
                        </div>
                        </div>';
                            ...
                            $id_form=$row1['id_formulaire'];
                            $sql2="SELECT * FROM questions WHERE id_formulaire='$id_form'";
                            $result2=mysqli_query($link,$sql2);

                            if ($result2==NULL){

                                echo '  <div class="d-flex justify-content-center mt-5">
                                        <p class="card-text text-danger fw-bold">Le formulaire de ce rôle est sans question, veuillez le modifier puis auditer. <a href="ajoutqst.php" class="text-success">Le modifier.</a></p>
                                        </div>
                                        </div>';
                            }else{
                                ...
                                while($row2=mysqli_fetch_assoc($result2)){
                                    ...
                                    $sql3="SELECT * FROM reponses WHERE id_question='$id_qst' and id_collaborateur='$id_collab'";
                                    $result3=mysqli_query($link,$sql3);
                                    if($result3==NULL){

                                        echo '  <div class="d-flex justify-content-center mt-5">
                                        <p class="card-text text-danger fw-bold">Pas de réponse disponible. <a href="auditer.php" class="text-success">Veuillez auditer.</a></p>
                                        </div>
                                        </div>';
                                    }else{
